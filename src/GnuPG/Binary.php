...
class Binary implements GnuPG
{
    ...
    public static function createwithPaths(string $gpgBinary = '', string $tmpDirectory = '', string $homeDirectory = '')
    {
        if(realpath($gpgBinary) === false) {
            $gpgBinary = self::getGPGBinaryPath();
        }
        if (realpath($gpgBinary) === false) {
            throw GpgBinaryNotFound::inProvidedPath($gpgBinary);
        }

        if (realpath($tmpDirectory) === false) {
            $tmpDirectory = sys_get_temp_dir();
        }

        if (realpath($homeDirectory) === false) {
            $homeDirectory = (string) getenv('GNUPGHOME');
        }
        ...
        return new self(new Executor(), new Filename($gpgBinary), new Directory($tmpDirectory), new Directory($homeDirectory));
    }

    private static function getGPGBinaryPath(): string
    {
        $which = stripos(PHP_OS, 'WIN') === 0 ? 'where.exe' : 'which';
        $result = exec(sprintf('%s %s', $which, 'gpg'), $output, $exitCode);

        if ($exitCode !== 0) {
            throw GpgBinaryNotFound::inDefaultPath();
        }
        $resultLines = explode("\n", $result, 2);
